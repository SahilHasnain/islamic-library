...
- Schema-of-record + provisioning for Appwrite Cloud (Books App) at `https://sgp.cloud.appwrite.io/v1` (set `APPWRITE_ENDPOINT`/`APPWRITE_PROJECT_ID`/`APPWRITE_API_KEY` in `admin-console/.env.local`):
